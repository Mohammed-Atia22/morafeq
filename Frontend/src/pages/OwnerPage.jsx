...
import { listingsApi } from "../features/listings/services/listingsApi";
    ...
    <div className="min-h-screen bg-[#eef3ff] text-[#172033]" dir="rtl">
      ...
      <div className="lg:pr-[260px]">
        ...
        <nav className="fixed bottom-0 inset-x-0 z-40 grid grid-cols-4 border-t border-slate-200 bg-white px-2 py-2 shadow-[0_-10px_30px_rgba(15,23,42,0.08)] lg:hidden">
          <MobileNavItem label="لوحة التحكم" icon={GridIcon} />
          <MobileNavItem label="عقاراتي" icon={BuildingIcon} active />
          <Link to="/AddListing" className="flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-xs font-black text-slate-500">
            <PlusCircleIcon className="h-5 w-5" />
            إضافة شقة
          </Link>
          <MobileNavItem label="الإعدادات" icon={SettingsIcon} />
        </nav>
      </div>
    </div>
  );
...
function OwnerSidebar({ user, logout }) {
  return (
    <>
      ...
      <nav className="flex-1 space-y-2 px-3 py-4">
        <SidebarItem label="لوحة التحكم" icon={GridIcon} />
        <SidebarItem label="عقاراتي" icon={BuildingIcon} active />
        <SidebarItem label="طلبات المعاينة" icon={EyeIcon} badge="5" />
        <Link to="/AddListing" className="flex h-12 items-center gap-3 rounded-xl px-4 text-sm font-bold text-slate-500 transition hover:bg-slate-50 hover:text-[#0b62d8]">
          <PlusCircleIcon className="h-5 w-5" />
          إضافة شقة
        </Link>
        <SidebarItem label="الرسائل" icon={MessageIcon} badge="11" danger />
        <SidebarItem label="الإعدادات" icon={SettingsIcon} />
      </nav>
...
function SidebarItem({ label, icon: Icon, active = false, badge, danger = false }) {
  return (
    <button
      type="button"
      ...
      className={[
        ...
        active ? "bg-[#e9f0ff] text-[#0b62d8]" : "cursor-default text-slate-500",
      ].join(" ")}
    >
...
function MobileNavItem({ label, icon: Icon, active = false }) {
  return (
    <button
      type="button"
      aria-current={active ? "page" : undefined}
      aria-disabled={!active}
      tabIndex={active ? 0 : -1}
      className={["flex flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-xs font-black", active ? "bg-[#e9f0ff] text-[#0b62d8]" : "cursor-default text-slate-500"].join(" ")}
    >
      <Icon className="h-5 w-5" />
      {label}
    </button>
