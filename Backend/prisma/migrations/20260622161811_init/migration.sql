...
CREATE TABLE `roommate_profiles` (
    ...
    `preferredMinRent` INTEGER UNSIGNED NULL,
    `preferredMaxRent` INTEGER UNSIGNED NULL,
    `preferredRoomType` ENUM('ENTIRE_PLACE', 'PRIVATE_ROOM', 'SHARED_ROOM') NULL,
    `preferredCity` VARCHAR(100) NULL,
    `preferredGovernorate` VARCHAR(100) NULL,
    `interests` JSON NULL,
...
CREATE TABLE `notifications` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `userId` INTEGER UNSIGNED NOT NULL,
    `type` ENUM('USER_VERIFICATION_APPROVED', 'USER_VERIFICATION_REJECTED', 'USER_VERIFICATION_NEEDS_CHANGES', 'LISTING_VERIFICATION_APPROVED', 'LISTING_VERIFICATION_REJECTED', 'LISTING_VERIFICATION_NEEDS_CHANGES', 'BOOKING_REQUESTED', 'BOOKING_APPROVED', 'BOOKING_REJECTED', 'PAYMENT_REQUIRED', 'PAYMENT_CONFIRMED') NOT NULL,
